...
Метою даного проєкту є побудова системи класифікації новин із використанням методів машинного та глибокого навчання,
...
словника до розробки архітектур і пояснення їхніх рішень, підкреслюючи важливість як точності, так і
...
- [5. Model explaining](notebook/5_model_explaining.ipynb)
...
Таким чином у ході експериментів було протестовано дві архітектури моделей deep learning:
...
На цьому етапі було здійснено інтерпретацію моделі (BiLSTM), з метою проаналізувати, які саме токени найбільше
...
Але спроби протестувати можливість виключення подібних токенів не призвели до покращення результатів.

### Порівняння моделей за метриками якості

| Модель                                   | F1 (Train) | F1 (Validation) | ROC-AUC (Train) | ROC-AUC (Validation) |
|------------------------------------------|------------|-----------------|-----------------|----------------------|
| **LogisticRegression + CountVectorizer** | 1.0000     | 0.9763          | 1.0000          | 0.9935               |
| **LogisticRegression + TF-IDF**          | 0.9830     | 0.9759          | 0.9985          | 0.9961               |
| **XGBModel + TF-IDF**                    | 0.9251     | 0.9180          | 0.9820          | 0.9758               |
| **FakeNewsRNNClassifier (GRU)**          | 0.9985     | 0.9589          | 0.9999          | 0.9946               |
| **FakeNewsLstmClassifier (BiLSTM)**      | **1.0000** | **0.9926**      | **1.0000**      | **0.9997**           |

### Короткі висновки:

- **BiLSTM** модель показує найкращу здатність до узагальнення на валідаційній вибірці згідно ROC-AUC.
- **GRU** дещо гірше справляється з генералізацією, хоча добре навчається.
- **TF-IDF** покращує ROC-AUC на валідації порівняно з CountVectorizer.
- Класичні моделі дають достойні результати, але менш гнучкі щодо контексту довгих текстів.
...
інтерпретованістю. При цьому окрема увага приділяється надійності моделей, уникненню data leakage і можливим
упередженням —
критично важливим аспектам для задач суспільного значення.

## Можливі покращення

- **Використання трансформерів (BERT, RoBERTa, DistilBERT):**  
  Сучасні трансформерні архітектури демонструють значно кращі результати на задачах класифікації тексту, оскільки здатні
  моделювати довгострокові залежності та семантичні зв’язки між словами. Їх використання дозволить:
    - обробляти текст як єдине ціле (без окремих енкодерів для title/text),
    - отримувати контекстно-залежні ембедінги,
    - покращити узагальнюваність та стійкість моделі до шаблонних атак.

### Обмеження та проблеми з датасетом:

- **Підозра на data leakage:**  
  Під час роботи з набором даних було виявлено, підозра на витік, що може завищувати метрики та не відображати реальну
  здатність моделі узагальнювати. Так навіть прості моделі демонстрували неприродно високі результати.

- **Сильна домінанта політичного контексту:**  
  Значна частина новин пов’язана з виборами в США та політичними фігурами. Це може зумовлювати упередження моделі та її
  неспроможність коректно працювати на новинах з інших доменів.

- **Наявність сенсаційної лексики як ключового маркера:**  
  Модель схильна спиратися на оціночну та емоційно забарвлену лексику, що добре працює в рамках датасету, але не
  гарантує адекватної поведінки в реальних умовах, де фейки можуть бути формально нейтральними.

Подальші експерименти з трансформерними моделями, усунення data leakage та більш ретельне формування датасету є
критичними кроками для переходу від експериментального прототипу до реального застосування в production-сценаріях.
